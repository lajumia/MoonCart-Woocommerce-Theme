...
var MoonCart = function(){
	...
	var handleCurrentActive = function() {
		for (var nk = window.location,
			o = jQuery("ul.navbar a").filter(function() {
	...
	var handleShopCart = function(){
		jQuery(".remove").on('click',function(){
			jQuery(this).closest(".mini_cart_item").hide('500');
		});
	...
	var priceslider = function(){
		if(jQuery("#slider-tooltips").length > 0 ) {
			...
			tooltipSlider.noUiSlider.on('update', function (values, handle, unencoded) {
				formatValues[0].innerHTML = "Min Price: " + "jQuery"+ values[0];
				formatValues[1].innerHTML = "Max Price: " + "jQuery"+ values[1];
			});
		}
	}
	...
	var handleBootstrapTouchSpin = function(){
		if(jQuery("input[name='demo_vertical2']").length > 0 ) {
	 	...
	 	if(jQuery(".quantity-input").length > 0 ) {
	...
	var dzCategoryToggle = function(){
		jQuery('.category-toggle .toggle-btn').on('click',function(){
			jQuery(".toggle-items").slideToggle("slow");
			jQuery(this).toggleClass('active');
		});
		
		jQuery('.browse-category-menu .category-btn').on('click',function(){
			jQuery(".category-menu-items").slideToggle("slow");
			jQuery(this).toggleClass('active');
		});
	...
	var heartBlast = function (){
		jQuery(".heart").on("click", function() {
			jQuery(this).toggleClass("heart-blast");
		});
	}
	...
	var handleShopPannel = function(){
		if(screenWidth <= 1199 ){
			jQuery(".panel-btn, .filter-top-btn").on('click',function(){
				jQuery(".shop-filter,.panel-close-btn").addClass('active');
			});
		}
		jQuery('.panel-close-btn').on('click',function(){
			jQuery(".shop-filter,.panel-close-btn").removeClass('active');
		})
		if(screenWidth >= 1199 ){
			jQuery("#filterTopBtn").on('click',function(){
				jQuery("#shopFilter").slideToggle("slow");
			});
		}
		jQuery(".btn-filter-left").on('click',function(){
			jQuery(".shop-filter, .panel-close-btn").addClass('active');
		});
	}
	
	var cartButton = function(){
		jQuery(".dz-close").on('click',function(){
			jQuery(this).closest(".sidebar-cart-list li").fadeOut("normal", function() {
				jQuery(this).remove();
			});
		});
		
		jQuery(".tag-close").on('click',function(){
			jQuery(this).closest(".filter-tag li").fadeOut("normal", function() {
				jQuery(this).remove();
			});
		});
	} 
	...
	var handleHeaderMenuItem = function(){
		jQuery(".menu-item").on('click',function(){
			jQuery(".dzdrop-menu").toggleClass('show');
		});
		jQuery(".menu-items").on('click',function(){
			jQuery(this).toggleClass('active');
		});
	}
	...
	var handleColorFilter = function(){
		...
		function colorChange(item, index, arr) {
			var color = jQuery(item).val();
			var element = jQuery(item).closest('.form-check').find('span');
			...
			if(screenWidth <= 768 ){
				if (jQuery(this).scrollTop() + 
					...
					document.querySelector("body").scrollHeight) { 
					 jQuery('.extra-nav').addClass('bottom-end');
				}else{
					 jQuery('.extra-nav').removeClass('bottom-end');
				}
				var currentScroll = jQuery(this).scrollTop();
				if (currentScroll > previousScroll){
					//scroll down
					jQuery('.extra-nav').addClass('active');
				} else {
					//scroll up
					jQuery('.extra-nav').removeClass('active');
				}
	...
	var handleMagnifyGallery = function(){
		...
		const imageSelector = jQuery('.DZoomImage');
		...
		imageSelector.on('mousemove',function(t) {
			let e = jQuery(this).offset();
			var i = (t.pageX - e.left) / jQuery(this).width() * 100 <= 100 ? (t.pageX - e.left) / jQuery(this).width() * 100 : 100;
			var c = (t.pageY - e.top) / jQuery(this).height() * 100 <= 100 ? (t.pageY - e.top) / jQuery(this).height() * 100 : 100;
			
			jQuery(this).find('img').css("transform-origin", i + "% " + c + "%");
		})
		imageSelector.on('mouseenter',function(t) {
			let n = jQuery(this).find('img');
			n.css("cursor", "pointer"), 
				/* n.css("width", jQuery(this).width() + "px"), 
				n.css("height", jQuery(this).height() + "px"), */ 
			n.css("transition", "0.1s"), 
			n.css("transform", "scale(" + 1.5 + ")"),
			jQuery(this).find('.mfp-link i').css({opacity : 1,zIndex : 1})
		});
		imageSelector.on('mouseleave',function(t) {
			let n = jQuery(this).find('img');
			n.css("transition", "0.1s"), n.css("transform", "scale(1)")
			jQuery(this).find('.mfp-link i').css({opacity : 0,zIndex : 1})
		});
	}
